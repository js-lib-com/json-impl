...
/**
 * Serialize primitive values, objects, arrays, collections and maps to JSON string representation. This class is invoked
 * internally by {@link JsonImpl} facade and is not intended to be reused; create a new serializer instance for every value to
 * stringify. Once instance created invoke {@link #serialize(Writer, Object)} with external created characters writer instance.
 * After serialization completes writer is flushed by left opened.
 * <p>
 * This class handle serialization in a best effort manner. If for some reason a field cannot be processed it is replaced by
 * null, but takes care to record failing condition to error log. Currently there are two conditions that could lead to
 * <code>null</code> replacement:
 * <ol>
 ...
 * </ol>
 * <p>
 * Serializer supports an extension used merely by this library: add class name before actual serialized value. Considering
 * below <code>js.test.Page</code> class:
 * 
 * <pre>
 * 	  class js.test.Page
 * 	  {
...
final class Serializer {
	...
	/**
	 * Serialize primitive or aggregated value to given writer. Writer is flushed after serialization completes but is left
	 * opened.
	 * 
	...
	/**
	 * Serialization worker implements core logic for serialization process. It handles both primitive and aggregated values.
	 * For aggregated values delegates helpers implemented by this class: {@link #serializeArray(Object)},
	 * {@link #serializeMap(Object)} and {@link #serializeObject(Object)}. Helpers method takes care of structural details but
	 * for value serialization re-invoke this method. If value is null uses {@link #KEYWORD_NULL}.
	 * <p>
	 * Because for non primitive values this method is executed recursively it implements protection against circular
	 * dependencies. When a circular dependency is discovered replace value with null and record to error log. For circular
	 * dependencies protection uses {@link #circularDependenciesStack}. Value to serialize is pushed to stack just before
	 * processing and extracted after. If value it is already in stack replace it with null and dump the stack to error log.
	 * 
	 * @param value primitive or aggregated value.
	 * @throws IOException if IO write operation fails.
	 */
	...
	/**
	 * Serialize value map entries separated by comma, key/value pair being separated by colon, all in curly braces. A map is
	 * serialized like an object in opening and closing curly braces. For key and value serialization this method recursively
	 * invoke {@link #serialize(Object)}.
	 * 
	...
	/**
	 * Serialize opening curly brace, all fields separated by comma and closing curly brace. All fields are serialized, in no
	 * particular order, except static and transient. If {@link #includeClass} flag is true fields serialization is preceded by
	 * value object class name. Also this method process superclass hierarchy as long as superclass is part of the same package
	 * as given value object.
	 * <p>
	 * Note that this method hides a recursive call to {@link #serialize(Object)} via {@link #serializeField(Object, Field)}.
	 * 
	 ...
	 * <p>
	 * Excerpt: ... string begins and ends with quotation marks. All Unicode characters may be placed within the quotation marks
	 * except for the characters that must be escaped: quotation mark, reverse solidus, and the control characters (U+0000
	 * through U+001F).
	 * 
	...
	/**
	 * Write a single character to internal JSON stream. This method is not only convenient but isolate JSON serializer logic
	 * from underlying JSON stream.
	 * 
