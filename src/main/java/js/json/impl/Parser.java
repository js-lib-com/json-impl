...
/**
 * Syntactic parser. Scans token generated by {@link Lexer} and initialize primitive values, object fields, array items or map
 * entries described by JSON stream. Note that this parser accepts only double quotes for delimiters mark, i.e. single quotation
 * mark is not supported; this limitation is actually implemented in lexer.
 * 
 * <h6>Extensions</h6>
 * <p>
 * This parser supports a proprietary extensions to JSON protocol. It accepts inband type information generated by
 * {@link Serializer#Serializer(boolean)}. Although this feature is public it is intended for library internal use only. Also it
 * accepts map keys of numeric and object types, see code snippet.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * In above example, JSON protocol specs requires string for property name but this parser accepts objects and numbers, both
 * integer and double. This extension is used for map instances deserialization and is supported by {@link Serializer} too.
 * 
 * <h6>Best Effort</h6>
 * <p>
 * This JSON parser is relaxed and follows a best effort approach. If a named property from JSON stream does not have the same
 * name field into target object, parser just warn on log and ignore. Also, fields from target object with no values into JSON
 * stream are set to null.
 * 
 * <h6>Undocumented Extension</h6>
 * <p>
 * JSON syntax mandates quotes for all string values no matter if used for property name or primitive value. This parser accepts
 * strings without quotes, of course provided string does not contain white space or reserved chars. For example an array of
 * enumerations is standard encoded ["LIGER","TIGON"] but this parser accepts [LIGER,TIGON]. Anyway, this extension is for
 * library internal use only and user space code is not encouraged to use it.
 * 
...
final class Parser {
	...
	/**
	 * Parse value of requested type, primitive or aggregated type, from JSON stream. This method prepare parser internal state
	 * and handle exception but delegates the hard work to {@link #_parse(Type)}. Also, for user code convenience, this method
	 * uses auto-cast, that is cast value to parameterized type internally avoiding the need for explicit cast. Is caller
	 * responsibility to ensure cast compatibility otherwise this method throws cast exception.
	 * 
	...
	/**
	 * Parse mixed types JSON array and return array instance with items initialized from stream. Stream should contain a JSON
	 * array that is not required to be homogeneous. Is caller responsibility to ensure JSON array from stream has desired
	 * number and types. If array from stream is smaller than the number of requested types, returned not yet initialized items
	 * are set to null.
	 * <p>
